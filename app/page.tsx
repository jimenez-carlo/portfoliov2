...
export default function Portfolio() {
  ...
  useEffect(() => {
    const handleGlobalClick = (e: MouseEvent) => {
      ...
      clickEffect.className = `global-click-effect text-${currentTheme.primary}`
  ...
  }, [currentTheme.primary, colorTheme])
  ...
  const techStack = [
    ...
    { name: "TypeScript", icon: "TS", color: `from-${currentTheme.primary} to-${currentTheme.primaryDark}` },
    ...
    { name: "Docker", icon: "🐳", color: `from-${currentTheme.primary} to-${currentTheme.primaryDark}` },
    { name: "Git", icon: "📝", color: "from-orange-600 to-red-600" },
    { name: "AWS", icon: "☁️", color: "from-orange-500 to-yellow-500" },
    { name: "Python", icon: "🐍", color: `from-${currentTheme.primary} to-yellow-500` },
  ]
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 relative overflow-hidden">
      ...
      <div className="fixed top-20 right-4 z-50">
        <div className="relative">
          <Button
            ...
            size="sm"
            className={`bg-${currentTheme.primary}/20 hover:bg-${currentTheme.primary} text-${currentTheme.text} hover:text-slate-900 border border-${currentTheme.border} font-mono text-xs px-3 py-2 transition-all duration-300 shadow-lg shadow-${currentTheme.glow} ripple-effect click-glow`}
          >
          ...
          {showThemeSelector && (
            <div
              className={`absolute top-full right-0 mt-2 bg-slate-900/95 backdrop-blur-sm border border-${currentTheme.border} rounded-lg p-2 space-y-2 shadow-2xl shadow-${currentTheme.glow} animate-in slide-in-from-top duration-300`}
            >
              <div className={`text-${currentTheme.text} font-mono text-xs mb-2 px-2`}>{">> SELECT_FACTION <<"}</div>
              ...
              {(Object.keys(themes) as ColorTheme[]).map((theme) => (
                ...
                >
                  ...
                  {colorTheme === theme && <span className={`text-${themes[theme].textLight}`}>●</span>}
                </button>
              ))}
            </div>
          )}
      ...
      {/* Navigation */}
      <nav
        className={`fixed top-0 left-0 right-0 z-50 bg-slate-950/95 backdrop-blur-sm border-b border-${currentTheme.border} shadow-lg shadow-${currentTheme.glow}`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3">
              <div className="relative">
                <div
                  className={`w-10 h-10 bg-gradient-to-br from-${currentTheme.primary} to-${currentTheme.primaryDark} rounded-lg flex items-center justify-center font-bold text-white text-lg border border-${currentTheme.primaryLight}/50 shadow-lg shadow-${currentTheme.glow} ripple-effect click-glow cursor-pointer`}
                >
                  CJ
                </div>
                <div className={`absolute -inset-1 bg-${currentTheme.primary}/20 rounded-lg blur-sm -z-10`}></div>
              </div>
              <div className="hidden sm:block">
                <span className={`text-${currentTheme.text} font-mono text-xs typing-effect`}>
                  {"< NEURAL_LINK_ESTABLISHED />"}
                </span>
                <div className={`text-${currentTheme.textLight} font-mono text-xs opacity-75`}>{systemStatus}</div>
              </div>
            </div>
            ...
            <div className="md:hidden">
              <Button
                ...
                className={`text-${currentTheme.text} hover:bg-${currentTheme.bg} border border-${currentTheme.border} ripple-effect click-glow`}
              >
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className={`md:hidden bg-slate-950/98 backdrop-blur-sm border-t border-${currentTheme.border}`}>
        ...
        <div className="text-center z-10 max-w-6xl mx-auto">
          <div className="flex flex-col lg:flex-row items-center justify-center gap-12 mb-8">
            ...
            <div className="relative order-2 lg:order-1 animate-fade-in-left">
              <div className="relative w-48 h-48 lg:w-56 lg:h-56">
                {/* Main portrait */}
                <div
                  className={`w-full h-full rounded-2xl overflow-hidden border-2 border-${currentTheme.border} shadow-2xl shadow-${currentTheme.glow} bg-slate-800 ripple-effect click-glow cursor-pointer`}
                >
                  <img
                ...
                <div className={`absolute -inset-2 bg-${currentTheme.primary}/10 rounded-2xl blur-xl -z-10`}></div>

                {/* StarCraft-style corner brackets */}
                <div
                  className={`absolute -top-2 -left-2 w-6 h-6 border-l-2 border-t-2 border-${currentTheme.primaryLight}`}
                ></div>
                <div
                  className={`absolute -top-2 -right-2 w-6 h-6 border-r-2 border-t-2 border-${currentTheme.primaryLight}`}
                ></div>
                <div
                  className={`absolute -bottom-2 -left-2 w-6 h-6 border-l-2 border-b-2 border-${currentTheme.primaryLight}`}
                ></div>
                <div
                  className={`absolute -bottom-2 -right-2 w-6 h-6 border-r-2 border-b-2 border-${currentTheme.primaryLight}`}
                ></div>
                ...
                {/* ID Badge */}
                <div
                  className={`absolute bottom-3 left-3 bg-slate-900/80 backdrop-blur-sm border border-${currentTheme.border} rounded px-2 py-1`}
                >
                  <span className={`text-${currentTheme.textLight} font-mono text-xs`}>ID: CJ_001</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 order-1 lg:order-2 text-center lg:text-left animate-fade-in-right">
              <div className="mb-6">
                <div className="inline-block relative">
                  <div
                    className={`w-20 h-20 lg:w-24 lg:h-24 mx-auto lg:mx-0 bg-gradient-to-br from-${currentTheme.primary} to-${currentTheme.primaryDark} rounded-full flex items-center justify-center text-4xl lg:text-5xl font-bold text-white mb-4 shadow-xl shadow-${currentTheme.glow} border-2 border-${currentTheme.primaryLight}/50 ripple-effect click-glow cursor-pointer`}
                  >
                    CJ
                  </div>
                  <div className={`absolute -inset-2 bg-${currentTheme.primary}/10 rounded-full blur-lg -z-10`}></div>
                  {/* Rotating ring */}
                  <div
                    className={`absolute inset-0 border-2 border-${currentTheme.primaryLight}/30 rounded-full animate-spin`}
                    style={{ animationDuration: "15s" }}
                  >
                    <div
                      className={`absolute top-0 left-1/2 w-1.5 h-1.5 bg-${currentTheme.primaryLight} rounded-full transform -translate-x-1/2 -translate-y-1`}
                    ></div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 mb-8">
                <div
                  className={`text-${currentTheme.text} font-mono text-sm tracking-wider typing-effect-slow matrix-text`}
                >
                  {">> NEURAL_INTERFACE_ACTIVE <<"}
                </div>
                <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold text-white mb-4 tracking-tight glitch-effect matrix-text">
                  CARLO JIMENEZ
                </h1>
                <div
                  className={`text-lg sm:text-xl lg:text-2xl text-${currentTheme.accent} font-mono tracking-wide matrix-text`}
                >
                  <span className={`text-${currentTheme.text}`}>{"["}</span>
                  SENIOR FULLSTACK DEVELOPER
                  <span className={`text-${currentTheme.text}`}>{"]"}</span>
                </div>
                <div
                  className={`text-${currentTheme.textLight} font-mono text-sm opacity-75 max-w-2xl mx-auto lg:mx-0 matrix-text`}
                >
                  SPECIALIZATION: QUANTUM_CODE_ARCHITECTURE | NEURAL_NETWORKS | SYSTEM_OPTIMIZATION
                </div>
              </div>

              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mb-6 matrix-text">
                <Button
                  onClick={() => scrollToSection("projects")}
                  className={`bg-${currentTheme.primary} hover:bg-${currentTheme.primaryHover} text-slate-900 font-semibold px-8 py-3 text-lg font-mono border border-${currentTheme.primaryLight}/50 shadow-lg shadow-${currentTheme.glow} transition-all duration-300 hover:shadow-${currentTheme.glowHover} ripple-effect click-glow`}
                >
                ...
                <Button
                  ...
                  variant="outline"
                  className={`border-${currentTheme.primary} text-${currentTheme.text} hover:bg-${currentTheme.primary} hover:text-slate-900 px-8 py-3 text-lg font-mono shadow-lg shadow-${currentTheme.glow}/10 transition-all duration-300 hover:shadow-${currentTheme.glow} ripple-effect click-glow`}
                >
                  {">> ESTABLISH_COMM_LINK"}
                </Button>
              </div>
              ...
              <div
                className={`inline-flex items-center space-x-2 text-${currentTheme.textLight} font-mono text-xs matrix-text`}
              >
                <div className={`w-2 h-2 bg-${currentTheme.primaryLight} rounded-full animate-pulse`}></div>
          ...
          <div className="mt-12 w-full max-w-4xl mx-auto animate-fade-in-up">
            <div className="text-center mb-6">
              <div className={`text-${currentTheme.text} font-mono text-xs mb-2 typing-effect`}>
                {">> ACTIVE_TECHNOLOGIES <<"}
              </div>
            </div>
            ...
            <div className="relative overflow-hidden tech-stack-fade">
              ...
              <div className="flex animate-scroll-infinite space-x-4 py-4">
                ...
                {techStack.map((tech, index) => (
                  <div
                    key={`first-${index}`}
                    className={`flex-shrink-0 w-24 h-24 bg-slate-800/50 border border-${currentTheme.border} rounded-lg backdrop-blur-sm hover:border-${currentTheme.borderHover} transition-all duration-300 group ripple-effect click-glow cursor-pointer relative`}
                  >
                    <div className="w-full h-full flex flex-col items-center justify-center p-2">
                      ...
                      <span className={`text-${currentTheme.textLight} font-mono text-xs text-center leading-tight`}>
                        {tech.name}
                      </span>
                    </div>
                    <div
                      className={`absolute -inset-1 bg-${currentTheme.primary}/5 rounded-lg blur-sm -z-10 group-hover:bg-${currentTheme.primary}/10 transition-all duration-300`}
                    ></div>
                  </div>
                ))}
          ...
          <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
            <div className="flex flex-col items-center space-y-2">
              <ChevronDown className={`w-6 h-6 text-${currentTheme.text}`} />
              <div className={`text-${currentTheme.text} font-mono text-xs typing-effect`}>SCROLL_TO_CONTINUE</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="about" className="py-20 px-4 sm:px-6 lg:px-8 relative" ref={(el) => (sectionRefs.current[1] = el)}>
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <div className={`text-${currentTheme.text} font-mono text-sm mb-2 typing-effect`}>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="relative animate-fade-in-left">
              <div
                className={`relative overflow-hidden rounded-lg border border-${currentTheme.border} shadow-2xl shadow-${currentTheme.glow} ripple-effect click-glow cursor-pointer`}
              >
                <img
                ...
                <div
                  className={`absolute bottom-4 left-4 text-${currentTheme.textLight} font-mono text-xs typing-effect`}
                >
            ...
            <div className="space-y-6 animate-fade-in-right">
              <div className={`border border-${currentTheme.border} rounded-lg p-6 bg-slate-900/50 backdrop-blur-sm`}>
                <div className={`text-${currentTheme.text} font-mono text-sm mb-3 typing-effect`}>
                  {">> BIOGRAPHICAL_DATA"}
                </div>
                <p className="text-slate-200 text-lg leading-relaxed mb-4 matrix-text">
                ...
                <p className="text-slate-200 text-lg leading-relaxed matrix-text">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div
                  className={`text-center p-4 border border-${currentTheme.border} rounded-lg bg-slate-900/30 ripple-effect click-glow cursor-pointer matrix-text`}
                >
                  <div className={`text-2xl font-bold text-${currentTheme.text}`}>5+</div>
                  <div className={`text-${currentTheme.accent} font-mono text-xs`}>YEARS_ACTIVE</div>
                </div>
                <div
                  className={`text-center p-4 border border-${currentTheme.border} rounded-lg bg-slate-900/30 ripple-effect click-glow cursor-pointer matrix-text`}
                >
                  <div className={`text-2xl font-bold text-${currentTheme.text}`}>50+</div>
                  <div className={`text-${currentTheme.accent} font-mono text-xs`}>MISSIONS_COMPLETED</div>
                </div>
                <div
                  className={`text-center p-4 border border-${currentTheme.border} rounded-lg bg-slate-900/30 ripple-effect click-glow cursor-pointer matrix-text`}
                >
                  <div className={`text-2xl font-bold text-${currentTheme.text}`}>99%</div>
                  <div className={`text-${currentTheme.accent} font-mono text-xs`}>SUCCESS_RATE</div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                {["PROBLEM_SOLVER", "TEAM_COORDINATOR", "CONTINUOUS_LEARNER"].map((trait, index) => (
                  <Badge
                    key={trait}
                    variant="secondary"
                    className={`bg-${currentTheme.bg} text-${currentTheme.textLight} border-${currentTheme.border} font-mono text-xs ripple-effect click-glow cursor-pointer matrix-text`}
            ...
            {["Frontend", "Backend", "Database", "Tools"].map((category, categoryIndex) => (
              ...
              >
                <CardHeader className="pb-3">
                  <CardTitle className={`text-${currentTheme.text} flex items-center gap-2 font-mono text-sm`}>
                ...
                <CardContent className="space-y-4">
                  {skills
                    ...
                    .map((skill, skillIndex) => (
                      <div
                        key={skill.name}
                        className="space-y-2 matrix-text"
                      ...
                      >
                        <div className="flex justify-between items-center text-sm">
                          ...
                          <span className={`text-${currentTheme.text} font-mono`}>{skill.level}%</span>
                        </div>
                        <div className="relative">
                          <div className="w-full bg-slate-700 rounded-full h-2 border border-slate-600">
                            <div
                              className={`bg-gradient-to-r ${currentTheme.gradient} h-2 rounded-full transition-all duration-1000 relative overflow-hidden`}
          ...
          <div className="grid lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-1">
              <div className={timelineSidebarSticky ? "sticky top-24" : "relative"}>
                ...
                >
                  <div className={`text-${currentTheme.text} font-mono text-sm mb-4 typing-effect`}>
                    {">> ACTIVE_DEPLOYMENT <<"}
                  </div>
                    ...
                    {workExperience.map((job, index) => (
                      ...
                      >
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          />
                          <div>
                            <div className={`text-${currentTheme.textLight} font-mono text-xs font-bold`}>
                              {job.company}
                            </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-700">
                    <div className={`text-${currentTheme.text} font-mono text-xs mb-2`}>MISSION_PROGRESS</div>
                    <div className="w-full bg-slate-700 rounded-full h-2">
                      <div
                        className={`bg-gradient-to-r ${currentTheme.gradient} h-2 rounded-full transition-all duration-500`}
                        style={{ width: `${((activeTimelineItem + 1) / workExperience.length) * 100}%` }}
                      ></div>
                    </div>
                    <div className={`text-${currentTheme.textLight} font-mono text-xs mt-1`}>
                  ...
                  {timelineSidebarSticky && (
                    <div className={`mt-4 pt-4 border-t border-slate-700`}>
                      <div className={`flex items-center gap-2 text-${currentTheme.textLight} font-mono text-xs`}>
                        <div className={`w-2 h-2 bg-${currentTheme.primaryLight} rounded-full animate-pulse`}></div>
            ...
            <div className="lg:col-span-3">
              <div className="relative">
                {/* Timeline line */}
                <div
                  className={`absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-${currentTheme.primary} via-${currentTheme.primaryLight} to-${currentTheme.primary} timeline-line opacity-30`}
                ></div>
                ...
                <div className="space-y-12">
                  {workExperience.map((job, index) => (
                    ...
                    >
                      {/* Timeline dot */}
                      <div
                        className={`absolute left-6 w-4 h-4 rounded-full border-2 border-${currentTheme.primary} ${
                          index === activeTimelineItem
                            ? `bg-${currentTheme.primary} shadow-lg shadow-${currentTheme.glow} timeline-pulse`
                            : "bg-slate-800"
                        }`}
                      ></div>
                        ...
                        >
                          <CardHeader>
                            <div className="flex items-start justify-between">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <CardTitle
                                    className={`text-${currentTheme.textLight} font-mono text-lg flex items-center gap-2`}
                                  >
                                    <Building2 className="w-5 h-5" />
                                    {job.company}
                                  </CardTitle>
                                  <div className={`text-${currentTheme.text} font-mono text-sm`}>{job.position}</div>
                          ...
                          <CardContent className="space-y-6">
                            <div>
                              ...
                              <p className="text-slate-200 leading-relaxed matrix-text">{job.description}</p>
                            </div>
                            ...
                            <div>
                              ...
                              <div className="space-y-2">
                                {job.technologies.map((tech, techIndex) => (
                                  <div
                                    key={techIndex}
                                    className={`flex items-center gap-2 text-slate-300 font-mono text-sm p-2 bg-slate-900/30 border border-slate-700 rounded matrix-text`}
                            ...
                            <div>
                              ...
                              <div className="space-y-2">
                                {job.achievements.map((achievement, achIndex) => (
                                  <div
                                    key={achIndex}
                                    className={`flex items-center gap-2 text-slate-300 font-mono text-sm matrix-text`}
                                    style={{ animationDelay: `${achIndex * 0.1}s` }}
                                  >
                                    <CheckCircle className={`w-4 h-4 text-${currentTheme.primaryLight}`} />
                                    {achievement}
                                  </div>
                                ))}
                              </div>
      ...
      <section id="projects" className="py-20 px-4 sm:px-6 lg:px-8" ref={(el) => (sectionRefs.current[4] = el)}>
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-center mb-8">
            <div className={`bg-slate-800/50 border border-${currentTheme.border} rounded-lg p-1 backdrop-blur-sm`}>
              <button
                ...
                className={`px-6 py-3 font-mono text-sm transition-all duration-300 rounded-md flex items-center gap-2 ripple-effect click-glow ${
                  activeProjectTab === "projects"
                    ? `bg-${currentTheme.primary} text-slate-900 shadow-lg shadow-${currentTheme.glow}`
                    : `text-${currentTheme.textLight} hover:bg-${currentTheme.bgHover} hover:text-${currentTheme.text}`
                }`}
              >
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {projects.map((project, index) => (
                ...
                >
                  ...
                  <CardHeader>
                    <CardTitle className={`text-${currentTheme.text} font-mono text-lg`}>{project.title}</CardTitle>
                    <CardDescription className="text-slate-200 matrix-text">{project.description}</CardDescription>
                  </CardHeader>
                ...
                {filteredGalleryImages.map((image, index) => (
                  <Dialog key={image.id}>
                    <DialogTrigger asChild>
                      <div
                        className={`group cursor-pointer bg-slate-800/50 border border-${currentTheme.border} rounded-lg overflow-hidden hover:border-${currentTheme.borderHover} transition-all duration-300 hover:shadow-lg hover:shadow-${currentTheme.glow} ripple-effect click-glow animate-fade-in-up`}
                        style={{ animationDelay: `${index * 0.1}s` }}
                      >
                        <div className="relative overflow-hidden">
                          <img
                          ...
                          <div className="absolute bottom-2 left-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <h3
                              className={`text-${currentTheme.textLight} font-mono text-xs font-bold mb-1 typing-effect`}
                            >
                              {image.title}
                            </h3>
                    ...
                    <DialogContent className="max-w-4xl w-full bg-slate-950/95 backdrop-blur-sm border-2 border-blue-500/30 p-0">
                      <div className="relative">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-950 via-slate-950/80 to-transparent p-6">
                          <h2
                            className={`text-${currentTheme.textLight} font-mono text-xl font-bold mb-2 typing-effect`}
                          >
                            {image.title}
                          </h2>
                          <p className="text-slate-300 text-sm matrix-text">{image.description}</p>
                          <Badge
      ...
      >
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div
            className={`border border-${currentTheme.border} rounded-lg p-8 bg-slate-800/30 backdrop-blur-sm mb-8 animate-fade-in-up`}
          >
            <p className="text-xl text-slate-200 mb-8 max-w-2xl mx-auto matrix-text">
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center items-center mb-8 matrix-text">
              <Button
                onClick={() => setShowContactModal(true)}
                className={`bg-${currentTheme.primary} hover:bg-${currentTheme.primaryHover} text-slate-900 font-semibold px-8 py-3 text-lg font-mono border border-${currentTheme.primaryLight}/50 shadow-lg shadow-${currentTheme.glow} transition-all duration-300 hover:shadow-${currentTheme.glowHover} ripple-effect click-glow`}
              >
            ...
            <div className={`text-${currentTheme.accent} font-mono text-sm space-y-2 matrix-text`}>
              <div className="flex items-center justify-center space-x-2">
                <div className={`w-2 h-2 bg-${currentTheme.primaryLight} rounded-full animate-pulse`}></div>
                <span className="typing-effect">NEURAL_LINK_STATUS: ACTIVE</span>
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="p-6">
              <DialogHeader className="mb-6">
                <DialogTitle className={`text-${currentTheme.textLight} font-mono text-xl font-bold typing-effect`}>
                  ESTABLISH_COMM_LINK
                </DialogTitle>
                <div className={`text-${currentTheme.text} font-mono text-sm typing-effect`}>
                  {">> SECURE_TRANSMISSION_PROTOCOL <<"}
                </div>
              </DialogHeader>

              <form className="space-y-4">
                <div className="space-y-2 matrix-text">
                  <Label htmlFor="name" className={`text-${currentTheme.textLight} font-mono text-sm`}>
                    OPERATIVE_NAME
                  </Label>
                  <Input
                    id="name"
                ...
                <div className="space-y-2 matrix-text">
                  <Label htmlFor="email" className={`text-${currentTheme.textLight} font-mono text-sm`}>
                    COMM_FREQUENCY
                  </Label>
                  <Input
                    id="email"
                ...
                <div className="space-y-2 matrix-text">
                  <Label htmlFor="subject" className={`text-${currentTheme.textLight} font-mono text-sm`}>
                    MISSION_TYPE
                  </Label>
                  <Input
                    id="subject"
                ...
                <div className="space-y-2 matrix-text">
                  <Label htmlFor="message" className={`text-${currentTheme.textLight} font-mono text-sm`}>
                    TRANSMISSION_DATA
                  </Label>
                  <Textarea
                    id="message"
                ...
                <Button
                  type="submit"
                  className={`w-full bg-${currentTheme.primary} hover:bg-${currentTheme.primaryHover} text-slate-900 font-mono text-sm py-3 ripple-effect click-glow matrix-text`}
                >
              ...
              <div className="mt-6 pt-6 border-t border-slate-700">
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div className={`p-3 bg-slate-800/30 border border-${currentTheme.border} rounded-lg matrix-text`}>
                    <Phone className={`w-5 h-5 text-${currentTheme.text} mx-auto mb-2`} />
                    <div className={`text-${currentTheme.textLight} font-mono text-xs mb-1`}>DIRECT_COMM</div>
                    <div className="text-slate-300 text-xs">+63 XXX XXX XXXX</div>
                  </div>
                  <div className={`p-3 bg-slate-800/30 border border-${currentTheme.border} rounded-lg matrix-text`}>
                    <MapPin className={`w-5 h-5 text-${currentTheme.text} mx-auto mb-2`} />
                    <div className={`text-${currentTheme.textLight} font-mono text-xs mb-1`}>COORDINATES</div>
            ...
            {/* Right side - Map */}
            <div className={`bg-slate-800/50 border-l border-${currentTheme.border} relative`}>
              <div className="h-full min-h-[500px] relative">
                ...
                <div className="absolute top-4 left-4 right-4">
                  <div
                    className={`bg-slate-900/90 backdrop-blur-sm border border-${currentTheme.border} rounded-lg p-4`}
                  >
                    <div className={`text-${currentTheme.textLight} font-mono text-sm font-bold mb-2 typing-effect`}>
                      TACTICAL_COORDINATES
                    </div>
                    <div className="text-slate-300 font-mono text-xs space-y-1 matrix-text">
                      <div>LAT: 14.5995° N</div>
                      <div>LON: 120.9842° E</div>
                      <div className={`text-${currentTheme.text}`}>STATUS: OPERATIONAL_ZONE</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  ...
                  >
                    <div
                      className={`text-${currentTheme.textLight} font-mono text-sm font-bold mb-2 flex items-center gap-2 typing-effect`}
                    >
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {availableDates.slice(0, 6).map((date, index) => (
                        <div
                          key={date}
                          className={`text-center p-2 bg-slate-800/50 border border-${currentTheme.border} rounded text-${currentTheme.textLight} font-mono text-xs matrix-text`}
                          style={{ animationDelay: `${index * 0.1}s` }}
                        >
      ...
      {showStickyUI && (
        <div
          className={`fixed bottom-0 left-0 right-0 z-40 bg-gradient-to-t from-slate-950 via-slate-900 to-slate-900/95 border-t-2 border-${currentTheme.border} backdrop-blur-sm shadow-2xl shadow-${currentTheme.glow} animate-in slide-in-from-bottom duration-500`}
        >
          <div className="max-w-7xl mx-auto px-4 py-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                <div
                  className={`w-16 h-16 bg-slate-800 border border-${currentTheme.border} rounded-lg p-1 relative overflow-hidden ripple-effect click-glow cursor-pointer`}
                >
                  <div
                    className={`w-full h-full bg-gradient-to-br from-${colorTheme}-900/50 to-slate-900 rounded border border-${currentTheme.primaryLight}/20 relative`}
                  >
                    <div
                    ...
                    ></div>
                    <div
                      className={`absolute top-1 left-1 w-2 h-2 bg-${currentTheme.primaryLight} rounded-full animate-pulse`}
                    ></div>
                    <div className="absolute bottom-1 right-1 w-1 h-1 bg-green-400 rounded-full"></div>
                  </div>
                  <div className={`absolute -inset-1 bg-${currentTheme.primary}/10 rounded-lg blur-sm -z-10`}></div>
                </div>

                <div className="space-y-1">
                  <div className={`text-${currentTheme.text} font-mono text-xs typing-effect`}>NEURAL_LINK_ACTIVE</div>
                  <div className={`text-${currentTheme.textLight} font-mono text-xs opacity-75`}>
              ...
              <div
                className={`flex items-center space-x-4 bg-slate-800/50 border border-${currentTheme.border} rounded-lg p-2 ripple-effect click-glow cursor-pointer`}
              >
                <div className="relative">
                  <div
                    className={`w-12 h-12 rounded-lg overflow-hidden border-2 border-${currentTheme.border} shadow-lg shadow-${currentTheme.glow}`}
                  >
                    <img
                      src="/images/carlo-portrait.png"
                ...
                <div className="space-y-1">
                  ...
                  <div className={`text-${currentTheme.textLight} font-mono text-xs`}>SR_FULLSTACK_DEV</div>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <div className="flex flex-col space-y-1 ml-4">
                  <div className="flex items-center space-x-2 text-xs font-mono">
                    <div className={`w-2 h-2 bg-${currentTheme.primaryLight} rounded-full`}></div>
                    <span className={`text-${currentTheme.textLight}`}>SKILLS: 12</span>
                  </div>
                  <div className="flex items-center space-x-2 text-xs font-mono">
          ...
          {/* Bottom accent line */}
          <div className={`h-1 bg-gradient-to-r from-transparent via-${currentTheme.primary} to-transparent`}></div>
        </div>
      )}
    </div>
  )
